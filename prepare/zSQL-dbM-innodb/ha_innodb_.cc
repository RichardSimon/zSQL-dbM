...
#include "string.h"     //z+  //dict0dict.h中需要用的memcpy等
#include "thr_lock.h"   //z+  //dict0dict.h中需要用的THR_LOCK //----2011-09-20-14-59;
#include "my_base.h"    //z+  //ha_rows //----2011-09-20-15-12;==15-17:R;
#include "dict0dict.h"  //z+ //dict_index_t
//#include "ha_innodb.h" //z- //----2011-09-20-15-20;
#include "ibuf0ibuf.h"  //z+ //IBUF_USE_COUNT
#include "i_s.h"
...
static char* innobase_independent_bp = NULL;
...
//static long		innobase_secondary_buffer_pool_instances = 4;
...
static PSI_file_info	all_innodb_files[] = {
	...
	{&innodb_file_sbp_data_key, "innodb_sbp_data_file", 0},
...
//static INNOBASE_SHARE *get_share(const char *table_name);  //z-
//static void free_share(INNOBASE_SHARE *share);  //z-
//static int innobase_close_connection(handlerton *hton, THD* thd);  //z-
//static int innobase_commit(handlerton *hton, THD* thd, bool all);  //z-
//static int innobase_rollback(handlerton *hton, THD* thd, bool all);  //z-
//static int innobase_rollback_to_savepoint(handlerton *hton, THD* thd,
//           void *savepoint);  //z-
//static int innobase_savepoint(handlerton *hton, THD* thd, void *savepoint);  //z-
...
static SHOW_VAR innodb_status_variables[]= {
  ...
  {"secondary_buffer_pool_flush",
  (char*) &export_vars.innodb_secondary_buffer_pool_pages_flush,	  SHOW_LONG},
  ...
  {"secondary_buffer_pool_migration_queue_sync",
  (char*) &export_vars.innodb_secondary_buffer_pool_migration_queue_flush,	  SHOW_LONG},
...
File my_open_osfhandle(HANDLE handle, int oflag){/*Associates a [file descriptor] with an existing [operating-system file handle].*/
  int offset= -1;
  uint i;
...
HANDLE my_get_osfhandle(File fd){/*Get Windows handle for a file descriptor*/
...
//typedef int		myf;	/* Type of MyFlags in my_funcs */  //=>my_global.h
//typedef int	File;		/* File descriptor */    //=>my_global.h
/*Create a temporary file with unique name in a given directory
  ...
  @note
    The behaviour of this function differs a lot between implementation, 
     it's main use is to generate a file with a name that does not already exist.
    When passing O_TEMPORARY flag in "mode"  the file should be automatically deleted
    [The implementation using mkstemp] should be considered the reference implementation when adding a new or modifying an existing one
*/
		      ...
		      myf MyFlags __attribute__((unused)))
{
  ...
  */
  if((file=my_open(to,  (mode & ~O_EXCL), MyFlags)) < 0){//=>my_winfile.c
  ...
  DBUG_RETURN(file);
} //=>mysys/mf_tempfile.c  //z //zlq
extern "C" 
int mysql_tmpfile(const char* prefix){
  char filename[FN_REFLEN];

#ifdef __WIN__  
  File fd = create_temp_file(filename, 
                             mysql_tmpdir, prefix,
                             O_CREAT | O_EXCL | O_RDWR | O_TEMPORARY  | O_BINARY | O_TRUNC | O_SEQUENTIAL | O_SHORT_LIVED,
                             MYF(MY_WME));//=>mysys/mf_tempfile.c(此文件就定义这么一个函数!)
#else
  File fd = create_temp_file(filename, 
                             mysql_tmpdir, prefix,
...
/*========================*/
{
  File	fd = mysql_tmpfile("ib");//=>sql/sql_class.cc
  int	fd2= -1;
  if(fd >= 0){
	...
	void	*p)	/*!< in: InnoDB handlerton */
{
	...
	if ( ut_strcmp(innobase_independent_bp,"") != 0 ){
		char str[512];
		ut_strcpy(str,innobase_independent_bp);
		ret = (bool) srv_parse_independent_bp_names_and_sizes(str);
	}
...
innobase_change_buffering_inited_ok:
	...
	srv_independent_bp = innobase_independent_bp;
...
/* plugin options */
static MYSQL_SYSVAR_STR(doublewrite_file, srv_doublewrite_file,
  PLUGIN_VAR_READONLY,
  "Doublewrite file name.",
  NULL, NULL, NULL);

static MYSQL_SYSVAR_ULONG(doublewrite_file_size, srv_doublewrite_file_size,
  PLUGIN_VAR_READONLY,
  "Doublewrite file size",
  NULL, NULL, 0, 0,4*1024*1024*1024 , 0);
...
static MYSQL_SYSVAR_ULONG(secondary_buffer_pool_instances, srv_sbp_instances,
  PLUGIN_VAR_RQCMDARG,
  "Number of Secondary Buffer Pool Instances ",
  NULL, NULL, 1, 0, 1, 0);
...
static MYSQL_SYSVAR_ULONG(secondary_buffer_pool_bufferd_writes, srv_sec_buf_pool_buffered_writes,
  ...
  NULL, NULL, 640, 256, 6400, 0);

static MYSQL_SYSVAR_ULONG(secondary_buffer_pool_mq_pct, srv_sec_buf_pool_mq_pct,
  PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
  "The write buffer queue in secondary buffer pool.",
  NULL, NULL, 20, 5, 80, 0);
...
static MYSQL_SYSVAR_BOOL(secondary_buffer_pool_enable_mq, srv_sec_buf_pool_enable_mq,
  PLUGIN_VAR_NOCMDARG | PLUGIN_VAR_READONLY,
  "Use migration queue with secondary buffer pool",
  NULL, NULL, TRUE);
...
static MYSQL_SYSVAR_STR(independent_bp, innobase_independent_bp,
  PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
  "Single buffer pool settings", NULL, NULL, "");

static struct st_mysql_sys_var* innobase_system_variables[]= {
  MYSQL_SYSVAR(secondary_buffer_pool_instances),
  MYSQL_SYSVAR(secondary_buffer_pool_bufferd_writes),
  MYSQL_SYSVAR(secondary_buffer_pool_enable_lru),
  MYSQL_SYSVAR(secondary_buffer_pool_enable_mq),
  MYSQL_SYSVAR(secondary_buffer_pool_mq_pct),
  ...
  MYSQL_SYSVAR(independent_bp),
  MYSQL_SYSVAR(doublewrite_file),
  MYSQL_SYSVAR(doublewrite_file_size),
