...
struct SyntaxNode_common{
  enum SyntaxNodeType        type;
  char*                      desc;
  struct SyntaxNode_common*  next;
};*/
typedef struct SyntaxNode_common;
...
struct TokenStruct{
  ...
  wchar_t* File;//+----2013-01-07-13-49;
  long Line;                          /* Line number in the input. */
  long Column;                        /* Column in the input. */
  struct SyntaxNode_common* SyntaxNode;//+----2013-01-12-14-51;
};
...
/* Exported functions. */
int Parse(wchar_t* InputBuf,         /* Pointer to the input data. */
...
wchar_t* RetrieveToken(wchar_t* InputBuf,  /* The input data. */
