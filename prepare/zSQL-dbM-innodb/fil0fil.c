...
/** Number of pending tablespace flushes */
UNIV_INTERN ulint	fil_n_pending_sbp_flushes	= 0;
...
add_size:
	...
	if (space->purpose == FIL_LOG) {
		...
		node->handle = os_file_create(innodb_file_data_key,
					      ...
					      OS_FILE_AIO, OS_DATA_FILE,
						     &ret);
	} else if (space->purpose == FIL_SBP){
		node->handle = os_file_create(innodb_file_sbp_data_key,
					      node->name, OS_FILE_OPEN,
					      OS_FILE_AIO, OS_DATA_FILE,
					      &ret);
		buf_sec_pool_ptr[space->id - SBP_TB_ID]->handle = node->handle;
	} else {
		node->handle = os_file_create(innodb_file_data_key,
					      node->name, OS_FILE_OPEN,
					      OS_FILE_AIO, OS_DATA_FILE,
	...
	if ((space->purpose == FIL_TABLESPACE || space->purpose == FIL_SBP) && space->id != 0) {
	...
	fil_system_t*	system)	/*!< in: tablespace memory cache */
{
	...
	if ( ( node->space->purpose == FIL_TABLESPACE  || node->space->purpose == FIL_SBP )
		&& node->space->id != 0) {
...
close_more:
	...
	if ( space_id == SBP_TB_ID){
		fil_flush_file_spaces(FIL_SBP);
	} else {
	...
	fil_space_t*	space)	/*!< in: space */
{
	...
	if (node->n_pending == 0 &&
		(space->purpose == FIL_TABLESPACE || space->purpose == FIL_SBP)
	    && space->id != 0) {
		/* The node is in the LRU list, remove it */
				...
				type == OS_FILE_WRITE */
{
	...
	if (node->n_pending == 0 &&
		(node->space->purpose == FIL_TABLESPACE || node->space->purpose == FIL_SBP)
	    && node->space->id != 0) {
				...
				aio used, else ignored */
{
	...
	if ( srv_sec_buf_pool_size > 0 ){
		if ( space_id == SBP_TB_ID && type == OS_FILE_WRITE )
			is_sbp = 1;
		//type & OS_FILE_SBP;
	}
...
# ifndef UNIV_LOG_DEBUG
	...
	/*ut_ad(!ibuf_inside() || is_log || (type == OS_FILE_WRITE)
	      || ibuf_page(space_id, zip_size, block_offset, NULL) || is_sbp);*/
# endif /* UNIV_LOG_DEBUG */
	...
	ut_ad((mode != OS_AIO_IBUF) || (space->purpose == FIL_TABLESPACE) || (space->purpose == FIL_SBP));
				...
				array to wait for */
{
	...
	} else if ( fil_node->space->purpose == FIL_SBP ){
		srv_set_io_thread_op_info(segment, "complete io for sbp");
		if(message != NULL){
			buf_sec_block_io_complete(message);
		}
	}
	else {
...
/*==============*/
{
	...
	while (fil_node != NULL /*&& fil_node->space->purpose != FIL_SBP*/) {
		ut_a(fil_node->n_pending == 0);
		ut_a(fil_node->open);
		ut_a(fil_node->space->purpose == FIL_TABLESPACE || fil_node->space->purpose == FIL_SBP);
	...
	ulint space_id)			/*!< in: tablespace space id */
{
		...
		if ( success ){
			...
			// Print preload table message to error file
			ut_print_timestamp(stderr);
			fprintf(stderr,"  InnoDB: preloading table %s.%s to secondary buffer pool.\n",dbname,tablename);

			/* Read the first page of the tablespace if the size big enough */
			buf2 = ut_malloc((20*FSP_EXTENT_SIZE+1) * UNIV_PAGE_SIZE);
			/* Align the memory for file i/o if we might have O_DIRECT set */
			page = ut_align(buf2, UNIV_PAGE_SIZE);

			for ( i = 0; i < (ib_uint64_t)((((((ib_uint64_t)size_high<<32)) + size) / UNIV_PAGE_SIZE)) ; i=i+20*FSP_EXTENT_SIZE ){
				// To support file greater than 4G
                foffset = ((ulint)(i*UNIV_PAGE_SIZE)) & 0xFFFFFFFFUL;
				foffset_high = (ib_uint64_t)(i*UNIV_PAGE_SIZE) >> 32;

				// Read page in disk from secondary buffer pool
				success = os_file_read_no_error_handling(file, page, foffset, foffset_high, UNIV_PAGE_SIZE*FSP_EXTENT_SIZE*20);
				if ( success == TRUE ){
					int j;
					for(j=0;j<FSP_EXTENT_SIZE*20;j++){
						if ( mach_read_from_2(page+j*UNIV_PAGE_SIZE+FIL_PAGE_TYPE) == FIL_PAGE_INDEX
							|| fil_page_get_type(page+j*UNIV_PAGE_SIZE) == FIL_PAGE_INODE ){
#ifdef UNIV_DEBUG
							ut_ad(i+j == mach_read_from_4(page+j*UNIV_PAGE_SIZE+FIL_PAGE_OFFSET));
#endif
							// Get secondary buffer pool
							buf_sec_pool = buf_sec_pool_get(space_id,i+j);
							ut_a(buf_sec_pool);
							HASH_SEARCH(hash,buf_sec_pool->page_hash,
												buf_page_address_fold(space_id,i+j),
												buf_sec_block_t*,block,
												ut_ad(1),
												block->space == space_id && block->offset == (i+j));
							if ( block ){
								...
								if ( UT_LIST_GET_LEN(buf_sec_pool->free) > 0 ){
									block = UT_LIST_GET_FIRST(buf_sec_pool->free);
									fil_io(OS_FILE_WRITE | OS_AIO_SIMULATED_WAKE_LATER, FALSE, SBP_TB_ID+buf_sec_pool->instance_no, 0,
										block->f_offset, 0, UNIV_PAGE_SIZE,
											(void*) (page+j*UNIV_PAGE_SIZE), NULL);
									block->offset = i+j;
									...
									for(j=0;j<srv_sbp_instances;j++){
										ret = ret + UT_LIST_GET_LEN(buf_sec_pool_ptr[j]->free);
									}
									if ( ret == 0 ){
										ut_print_timestamp(stderr);
										...
										os_aio_simulated_wake_handler_threads();
										os_aio_wait_until_no_pending_sbp_writes();
										fil_flush_file_spaces(FIL_SBP);
										ut_free(buf2);
										return (FALSE);
									}
...
/********************************************************************//**
Preloading the tablespaces to secondary buffer pool*/
UNIV_INTERN
void
		...
		if (dbdir != NULL) {
			...
			while (ret == 0) {
				...
				if (strlen(fileinfo.name) > 4
					...
					if  (fil_pre_load_to_secondary_buffer_pool(file,dbinfo.name,strtok(fileinfo.name,"."),space_id) == FALSE )
						goto finish;
...
finish:
	...
	//	os_file_flush(buf_sec_pool_ptr[i]->handle);
	//if ( !srv_sec_buf_pool_direct_io ){
	//	os_file_flush(buf_sec_pool_ptr[0]->handle);
	//}
	mem_free(dbpath);
}
