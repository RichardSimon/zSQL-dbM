...
UNIV_INTERN mysql_pfs_key_t  innodb_file_sbp_data_key;
...
static os_aio_array_t*	os_aio_sbp_array	= NULL; /*!< Secondary buffer pool I/O */
/* @} */
...
#endif
#ifdef USE_FILE_LOCK
//#include <bits/fcntl.h> //z+ //为了"struct flock" //----2011-09-20-12-10--12-18;
#include <fcntl.h>
/****************************************************************//**
...
//#include "test.c" //-2011-10-08-16-31;
/***********************************************************************//**
...
{
	FILE*	file	= NULL;
	int	fd	= innobase_mysql_tmpfile();  //----2011-10-08-16-20--:OK;
//int fd; //----2011-09-21-12-21:
//int fd=0; //“2011年09月21日 星期三 12时21分27秒”；;
...
/*		ret = os_file_write(name, file, buf,
				    ...
				    n_bytes);*/ //----2011-10-09-19-26://zlqlxm
ret = os_file_write(name, file, buf,
				    (ulint)(current_size & 0xFFFFFFFF),
				    (ulint)0,
				    n_bytes);
				...
				array */
{
	...
	if ( srv_sec_buf_pool_size > 0 ){
		n_segments = 3 + n_read_segs + n_write_segs;
	}else{
	...
	if ( srv_sec_buf_pool_size > 0){
		for (i = 2 + n_read_segs; i < n_segments - 1; i++) {
			ut_a(i < SRV_MAX_N_IO_THREADS);
			srv_io_thread_function[i] = "write thread";

		}
		srv_io_thread_function[i] = "sbp write thread";
		os_aio_sbp_array = os_aio_array_create(n_per_seg,1);
	}else{
...
/*=============*/
{
	...
	if ( srv_sec_buf_pool_size > 0 ){
		os_aio_array_free(os_aio_sbp_array);
		os_aio_sbp_array = NULL;
	}
...
/*=====================================*/
{
	...
	if ( srv_sec_buf_pool_size > 0 ){
		os_aio_array_wake_win_aio_at_shutdown(os_aio_sbp_array);
	}
#elif defined(LINUX_NATIVE_AIO)
...
UNIV_INTERN
void
os_aio_wait_until_no_pending_sbp_writes(void)
/*=====================================*/
{
	os_event_wait(os_aio_sbp_array->is_empty);
}
	...
	ulint*	type)		/*!< out: OS_FILE_WRITE or ..._READ */
{
	...
	if (array == os_aio_sync_array) {
		WaitForSingleObject(
			...
			INFINITE);
		i = pos;
	} /*else if ( array == os_aio_sbp_array ){
		i = pos;
	}*/else {
