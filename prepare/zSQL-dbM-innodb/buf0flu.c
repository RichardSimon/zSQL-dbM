...
/********************************************************************//**
Flush a batch of writes to the sbp that have already been
written by the OS. */
static
void
sbp_flush_sync_datafiles(void)
/*==========================*/
{
...
UNIV_INTERN
ulint
buf_flush_sbp_buffered_writes(ulint instance_no){
	buf_sec_pool_t* buf_sec_pool;
    buf_sec_block_t* start;
	buf_sec_block_t* end;
	buf_sec_block_t* b;
	buf_sec_block_t* d;
    ulint     len = 0;
	...
	buf_sec_pool = buf_sec_pool_ptr[instance_no];

retry:
	//mutex_enter(&buf_sec_pool->mutex);
	mutex_enter(&buf_sec_pool->flush_list_mutex);

    len = UT_LIST_GET_LEN(buf_sec_pool->flush_list);
	if ( len == 0 ){
		mutex_exit(&buf_sec_pool->flush_list_mutex);
		//mutex_exit(&buf_sec_pool->mutex);
        return 0;
	}

	ut_a( buf_sec_pool->n_flush == 0 );

	//if ( buf_sec_pool->n_flush == 0 ){
	//	os_event_reset(buf_sec_pool->no_flush);
	//}

	start = UT_LIST_GET_FIRST(buf_sec_pool->flush_list);
	end = UT_LIST_GET_LAST(buf_sec_pool->flush_list);
	b = start;
	//buf_sec_pool->n_flush = len;
	count = len;
	mutex_exit(&buf_sec_pool->flush_list_mutex);
	//mutex_exit(&buf_sec_pool->mutex);
...
retry2:
		...
		mutex_enter(&b->mutex);
		if ( b->io_fix == BUF_IO_READ ){
			ut_print_timestamp(stderr);
#ifdef UNIV_DEBUG
			fprintf(stderr," [Warning]: flushing while sbp is reading.\n");
#endif
			mutex_exit(&b->mutex);
			mutex_exit(&buf_sec_pool->mutex);
			//mutex_exit(&buf_sec_pool->mutex);
			...
			ut_a( b->io_fix == BUF_IO_NONE );
			b->io_fix = BUF_IO_WRITE;
			buf_sec_pool->n_flush++;
		}
		d = UT_LIST_GET_NEXT(flush_list,b);
		mutex_exit(&b->mutex);
		mutex_exit(&buf_sec_pool->mutex);

		fil_io(OS_FILE_WRITE | OS_AIO_SIMULATED_WAKE_LATER, FALSE, SBP_TB_ID+buf_sec_pool->instance_no, 0,
										b->f_offset, 0, UNIV_PAGE_SIZE,
										b->bw->frame, b);
		//fil_io(OS_FILE_WRITE, TRUE, SBP_TB_ID+buf_sec_pool->instance_no, 0,
		//								b->f_offset, 0, UNIV_PAGE_SIZE,
		//								b->bw->frame, NULL);
		//buf_sec_block_io_complete(b);
		b = d;
		count--;
	}

	sbp_flush_sync_datafiles();

	//thd_wait_begin(NULL, THD_WAIT_DISKIO);
	os_event_wait(buf_sec_pool->no_flush);
	//thd_wait_end(NULL);
...
corrupted_page:
		...
		//	buf_sec_pool_t* sbp;
		//	sbp = buf_sec_pool_get(((buf_page_t*)block)->space,((buf_page_t*)block)->offset);
		//	if (mutex_enter_nowait(&sbp->mutex) == 0 ){
		//		dw_flush++;
		//		sbp->stat.n_dw_flush++;
	...
	fil_io(OS_FILE_WRITE, TRUE, 
		(srv_doublewrite_file_size>0?DOUBLEWRITE_SPACE_ID:TRX_SYS_SPACE),0,
...
flush:
	...
	fil_flush((srv_doublewrite_file_size>0?DOUBLEWRITE_SPACE_ID:TRX_SYS_SPACE));
	...
	for (i = 0; i < trx_doublewrite->first_free; i++) {
		...
		if ( srv_sec_buf_pool_size > 0 ){
			...
			buf_sec_pool = buf_sec_pool_get(block->page.space,block->page.offset);
retry:
			mutex_enter(&buf_sec_pool->mutex);
			...
			if ( b ){
				mutex_enter(&b->mutex);
				if ( b->bw ){
					if ( b->io_fix == BUF_IO_NONE ){
						/* find in flush list, and not in flushing state, just copy to frame list */
						mutex_exit(&b->mutex);
						mutex_exit(&buf_sec_pool->mutex);
						ut_memcpy(b->bw->frame,block->frame,UNIV_PAGE_SIZE);
						mutex_enter(&buf_sec_pool->mutex);
						buf_sec_pool->stat.n_page_flush++;
						mutex_exit(&buf_sec_pool->mutex);
					}
					else if ( b->io_fix == BUF_IO_WRITE ){
						/* find in flush list, and is flushing, retry */
						mutex_exit(&b->mutex);
						mutex_exit(&buf_sec_pool->mutex);
						goto retry;
					}
					...
					ut_a(b->io_fix != BUF_IO_READ && b->io_fix != BUF_IO_WRITE);		
					/* find in LRU, remove from LRU list */
					...
					or BUF_FLUSH_LIST */
{
	...
	if (buf_pool == NULL) {
		...
		for (i = 0; i < srv_buf_pool_instances; ++i) {
			...
			//thd_wait_begin(NULL, THD_WAIT_DISKIO);  //- //因为上面的“#include "mysql/plugin.h" .. #include "mysql/service_thd_wait.h"”文件不存在！；
			os_event_wait(buf_pool->no_flush[type]);
			//thd_wait_end(NULL);  //-
		}
	} else {
	        //thd_wait_begin(NULL, THD_WAIT_DISKIO);  //z-
		os_event_wait(buf_pool->no_flush[type]);
		//thd_wait_end(NULL);  //z-
	}
}
